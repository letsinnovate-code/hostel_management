...
export default function EditHostelPage() {
  ...
  const loadHostel = async () => {
    ...
    try {
      ...
      const hostelData = response?.data ?? response;
      setHostel(hostelData);
      populateFormFromHostel(hostelData);
    } catch (error: any) {
  ...
  const populateFormFromHostel = (hostelData: any) => {
    setFormData({
      ...
      address: {
        ...
        coordinates: hostelData.address?.coordinates || null,
        formattedAddress: hostelData.address?.formattedAddress || '',
        placeId: hostelData.address?.placeId || '',
      },
      contact: {
  ...
  const handleUploadImages = async () => {
    ...
    try {
      ...
      await api.uploadHostelImages(id, formDataImages);
      setImageFiles([]);
      await loadHostel();
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      showToast(error.message || 'Failed to update hostel', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
      ...
      <div className="p-6">
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          {/* Form Content - Same as create page */}
          <div className="p-6">
            {activeSection === 'basic' && (
            ...
            {activeSection === 'images' && (
              <div className="space-y-4">
                <p className="text-sm text-gray-600">
                  Add or remove hostel photos. First image is used as cover. Upload new images below.
                </p>
                {hostel && Array.isArray(hostel.images) && hostel.images.filter((img: string) => img?.trim()).length > 0 && (
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-gray-700">Current images</p>
                    <div className="flex flex-wrap gap-3">
                      {(hostel.images as string[]).filter((img) => img?.trim()).map((url: string, index: number) => (
                        <div key={url} className="relative group">
                          ...
                          <button
                            ...
                            className="absolute top-1 right-1 w-6 h-6 flex items-center justify-center bg-red-500 text-white rounded-full text-sm opacity-0 group-hover:opacity-100 transition-opacity"
                          >
                            ×
                          </button>
                        </div>
                ...
                {imageFiles.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-gray-700">New images to upload ({imageFiles.length})</p>
                    <div className="flex flex-wrap gap-2">
                    ...
                    >
                      {uploadingImages ? 'Uploading...' : 'Upload these images'}
                    </button>
                  </div>
                )}
              </div>
            ...
            {activeSection === 'address' && formData && (
              <div className="space-y-4">
                <AddressInput
                  ...
                  onChange={(address, coordinates, placeId) => {
                    ...
                    setFormData({
                      ...
                      address: {
                        ...
                        coordinates: coordinates ? { latitude: coordinates.latitude, longitude: coordinates.longitude } : formData.address.coordinates,
            ...
            {activeSection === 'amenities' && formData && (
            ...
            {activeSection === 'facilities' && formData && (
              <div className="space-y-6">
                <div className="space-y-4">
            ...
            {activeSection === 'business' && formData && (
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
