    ...
    for (const student of eligibleStudents) {
      ...
      if (initialStatus === 'OUTSIDE_GRACE') {
        ...
        Notification.create({
          ...
          console.error('[CurfewAutomationService] Failed to create grace period notification:', err.message);
        });
      }
    }
        ...
        if (st.status === 'OUTSIDE_GRACE' && st.graceDeadline && now >= new Date(st.graceDeadline)) {
          ...
          if (!st.idempotentKeys.includes(violationKey)) {
            ...
            await CurfewViolation.create({
              ...
              console.error('[CurfewAutomationService] Failed to create CurfewViolation record:', err.message);
            });
            ...
            Notification.create({
              ...
              console.error('[CurfewAutomationService] Failed to create curfew violation notification:', err.message);
            });
        ...
        if (st.status === 'VIOLATION' && st.secondCountdownDeadline && now >= new Date(st.secondCountdownDeadline)) {
          ...
          if (!st.idempotentKeys.includes(wardenOwnerKey)) {
            ...
            await HostelAlertService.send({
              ...
              console.error('[CurfewAutomationService] Failed to dispatch HostelAlertService escalation alert:', err.message);
            });
