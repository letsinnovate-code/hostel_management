...
class OccupancyService {
  ...
  static async getOccupancy(hostelId, forceRefresh = false) {
    ...
    // Try cache first (transient cache; fallback to DB if unavailable)
    if (redis && !forceRefresh) {
      try {
        const cached = await redis.get(cacheKey);
        if (cached) return JSON.parse(cached);
      } catch (cacheErr) {
        // Redis cache read failure is intentionally non-fatal; falls back to DB computation
        console.warn('[OccupancyService] Redis cache read failed, falling back to database:', cacheErr.message);
      }
    }
    ...
    if (redis) {
      try {
        ...
        // Redis cache write failure is intentionally non-fatal; DB data is authoritative
        console.warn('[OccupancyService] Redis cache write failed:', cacheErr.message);
      }
    }
  ...
  static async checkOccupancyLimits(hostelId) {
    try {
      ...
      if (redis) {
        try {
          await redis.del(`occupancy:${hostelId}`);
        } catch (cacheErr) {
          // Redis cache eviction failure is non-fatal; TTL ensures eventual consistency
          console.warn('[OccupancyService] Redis cache invalidation failed:', cacheErr.message);
        }
      }
