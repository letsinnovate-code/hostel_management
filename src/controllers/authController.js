...
// @access  Private
exports.setCurrentRole = asyncHandler(async (req, res) => {
  const { role } = req.body;
  const user = await User.findById(req.user.id);
...
exports.registerStudentViaQR = asyncHandler(async (req, res) => {
  const {
    name,
    email,
    phone,
...
exports.changePassword = asyncHandler(async (req, res) => {
  ...
  user.tokenVersion = (user.tokenVersion || 0) + 1; // Invalidate all existing tokens on password change (TASK 1)
  await user.save();
...
// @desc    Invalidate all active sessions for current user across all devices
