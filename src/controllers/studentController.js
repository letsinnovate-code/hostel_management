...
const CHECKOUT_COOLDOWN_MS = COOLDOWNS.CHECKOUT_COOLDOWN_MS;
const NOTIFICATION_DEDUPE_WINDOW_MS = COOLDOWNS.NOTIFICATION_DEDUPE_WINDOW_MS;
...
exports.getProfile = async (req, res) => {
  try {
    ...
    let onboarding = null;
    try {
      onboarding = await StudentOnboarding.findOne({ studentId: req.user.id })
        .populate('hostelId')
        .populate('roomId')
        .lean();
    } catch (onbErr) {
      console.warn('[studentController] Error loading onboarding:', onbErr.message);
    }

    const rules = await require('../models/Rule').find({
      hostelId: user?.hostelId?._id || user?.hostelId || onboarding?.hostelId?._id || onboarding?.hostelId,
      isActive: true,
    });
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (updatePayload.parentInfo && !updatePayload.parentContact) {
      updatePayload.parentContact = updatePayload.parentInfo;
    } else if (updatePayload.parentContact && !updatePayload.parentInfo) {
      updatePayload.parentInfo = updatePayload.parentContact;
    }
    ...
    // Also synchronize onboarding record if present
    try {
      const onboarding = await StudentOnboarding.findOne({ studentId: req.user.id });
      if (onboarding) {
        if (updatePayload.name) onboarding.personalInfo = { ...(onboarding.personalInfo || {}), fullName: updatePayload.name };
        if (updatePayload.phone) onboarding.personalInfo = { ...(onboarding.personalInfo || {}), phone: updatePayload.phone };
        if (updatePayload.address) onboarding.personalInfo = { ...(onboarding.personalInfo || {}), address: updatePayload.address };
        if (updatePayload.parentInfo || updatePayload.parentContact) {
          onboarding.parentInfo = { ...(onboarding.parentInfo || {}), ...(updatePayload.parentInfo || updatePayload.parentContact) };
        }
        await onboarding.save();
      }
    } catch (onbErr) {
      console.warn('[studentController] Error updating onboarding sync:', onbErr.message);
    }
...
exports.getAttendanceAnalytics = async (req, res) => {
  try {
    ...
    const data = await AttendanceAnalyticsService.calculateStudentAttendanceAnalytics(req.user.id, {
      period,
...
exports.getPermissionRequests = async (req, res) => {
  try {
    ...
    const { permissions, total } = await PermissionService.listPermissions({
...
exports.cancelPermissionRequest = async (req, res) => {
  try {
    ...
    const { clientIp, userAgent } = extractClientMetadata(req);
    const updated = await PermissionService.cancelPermission(req.params.id, {
      user: req.user,
      ip: clientIp,
      userAgent,
      isStudent: true,
...
exports.getEmergencyHistory = async (req, res) => {
  try {
    ...
    const [total, emergencies] = await Promise.all([
      Emergency.countDocuments({ raisedBy: req.user.id }),
      Emergency.find({ raisedBy: req.user.id })
...
exports.getVisitorRequests = async (req, res) => {
  try {
    ...
    const { visitors, total } = await VisitorService.listVisitors({
      filter: { visitingStudentId: req.user.id },
      skip,
      limit,
