...
// Helper: Secure document URL validation (delegated to OnboardingService)
function validateDocumentUrl(url) {
  return OnboardingService.validateDocumentUrl(url);
}
  ...
  try {
    ...
    if (onboarding.hostelId) {
      Notification.create({
        ...
        console.error('[onboardingController] Asynchronous operation failed:', err.message);
      });
  ...
  try {
    ...
    if (student) {
      ...
      if (student.email) {
        ...
        sendApprovalEmail(student.email, student.name, hostelName).catch((err) => {
        console.error('[onboardingController] Asynchronous operation failed:', err.message);
      });
      }
    }
