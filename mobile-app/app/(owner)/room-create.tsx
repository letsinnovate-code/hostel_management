...
export default function RoomCreateScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const list = await api.getHostels();
        const arr = Array.isArray(list) ? list : list?.data ?? [];
        setHostels(arr);
        if (arr.length > 0 && !selectedHostelId) setSelectedHostelId(String(arr[0]._id || arr[0].id));
      } catch (_) {}
  ...
  const removeImage = (i: number) => setImageUris((prev) => prev.filter((_, idx) => idx !== i));

  const uploadPickedImages = async (roomId: string) => {
    ...
    try {
      ...
      await api.uploadRoomImages(roomId, fd);
    } catch (_) {
      Alert.alert('Upload warning', 'Room created but some images failed. You can add them from room detail.');
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (roomId && imageUris.length > 0) await uploadPickedImages(roomId);
      Alert.alert('Success', 'Room created successfully', [
        { text: 'View Room', onPress: () => router.replace(`/(owner)/room-detail/${roomId}` as any) },
        { text: 'Back to List', onPress: () => router.replace('/(owner)/rooms' as any) },
      ]);
    } catch (e: any) {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Add a new room to your hostel</Text>
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.section}>
            ...
            >
              <Text style={styles.pickerText}>
                {hostelsLoading ? 'Loading...' : hostels.find((h) => (h._id || h.id) === selectedHostelId)?.name ?? 'Select hostel'}
              </Text>
            ...
            <TextInput
              ...
              placeholder="Room description, features..."
              placeholderTextColor="#94a3b8"
              multiline
              numberOfLines={3}
            />
        ...
        {activeTab === 'images' && (
          <View style={styles.section}>
            <Text style={styles.label}>Room photos (optional)</Text>
