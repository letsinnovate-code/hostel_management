...
export default function StudentSupportScreen() {
  ...
  const loadTickets = async () => {
    setLoading(true);
    try {
      const response = await api.getStudentSupportTickets();
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.createStudentSupportTicket({
      ...
      >
        ...
        {tickets.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySubtext}>Create a ticket to get help from support</Text>
          </View>
        ) : (
          tickets.map((ticket) => (
            <View key={ticket._id} style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.ticketNumber}>
                  {ticket.ticketNumber || `#${String(ticket._id).slice(-6)}`}
                </Text>
                <View
                  style={[
                ...
                >
                  <Text style={styles.priorityText}>{(ticket.priority || 'medium').toUpperCase()}</Text>
                </View>
              </View>
              ...
              <View style={styles.footer}>
                ...
                >
                  <Text style={styles.statusText}>{(ticket.status || 'open').toUpperCase()}</Text>
                </View>
